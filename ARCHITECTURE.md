...
- **A claim carries no reason, so it is not a gate.** The property above is the whole of what a claim asserts: *someone took this*. It does not say who is waiting, or whether anything was produced, and nothing in the filename can. That makes a claim held deliberately — as a hold, while its item waits on a human — **indistinguishable to any sweeper** from one left behind by a dead agent. On 2026-08-07 a sweeper reaping claims from a daemon bounce checked each one individually for a pushed branch and a merged commit, which is a sound test for a build ticket and a vacuous one for a gh-issue triage whose entire deliverable is a packet appended to the ticket body. Five completed triages went back to `available/`; the next dispatch onto one would have re-run it over the body holding its only copy. The instrument was blind to the class, not carelessly applied. **The field that does carry a reason is `assignee`** — `human`, `parked`, `blocked:<agent>` are what pogo's dispatcher gates on by default, through one predicate with two enforcement points — and it is on the item, in plain text, where a sweeper that knows nothing about triage protocols still reads it. So `mg unclaim --assignee=WHO` writes it *before* the release rather than after: the two-command form leaves a window in `available/` with no assignee, and mg-24d2 spent 2m57s in exactly that window being named by the priority-wake. What a convention can add on top — a `stage: gated` line only the protocol that wrote it understands — is not a signal.
