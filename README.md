...
| `mg new --declares-remainder` | Marks an item whose **output is a recommendation**: a triage verdict, a design, a proposal. Its build is undone by construction the moment it completes, so `mg done` refuses the item until a successor names what carries it forward. mg writes the `declares-remainder` tag itself, in one canonical spelling, so the declaration cannot be misspelled or half-remembered. Add it to an item that already exists with `mg edit <id> --add-tags=declares-remainder`; retract a declaration that turned out to be wrong (a triage that concludes nothing is owed) with `--rm-tags=declares-remainder`. |
